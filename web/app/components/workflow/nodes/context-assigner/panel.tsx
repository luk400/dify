...
import React from 'react'
import { useTranslation } from 'react-i18next'
import { RiArrowDownSLine, RiAddLine, RiDeleteBinLine } from '@remixicon/react'
import useConfig from './use-config'
import type { ConversationManagerNodeType, OperationType, RoleType, WhichType, HowType } from './types'
import Field from '@/app/components/workflow/nodes/_base/components/field'
import Split from '@/app/components/workflow/nodes/_base/components/split'
...
const Panel: FC<NodePanelProps<ConversationManagerNodeType>> = ({
  ...
  return (
    <div className='mt-2'>
      <div className='px-4 pb-4 space-y-4'>
        <Field title="Conversation Variables">
          <div className="flex flex-col gap-2">
            {inputs.conversation_variables && inputs.conversation_variables.map((variable, index) => (
              <div key={index} className="flex items-center gap-2">
                <VarReferencePicker
                  ...
                  isShowNodeName
                  value={variable}
                  onChange={(newVar) => handleVarReferenceChange(newVar, index)}
                  filterVar={filterContextVar}
                  placeholder={t('workflow.nodes.assigner.selectVariable') as string}
                  className="flex-1"
                />
                <ActionButton 
            ...
            >
              ...
              <span className="text-text-secondary text-xs">Add Variable</span>
            </ActionButton>
          </div>
        </Field>
