...
//import type { VarGroupItem, ContextAssignerNodeType } from './types'
...
//
//const useConfig = (id: string, payload: ContextAssignerNodeType) => {
//  const { nodesReadOnly: readOnly } = useNodesReadOnly()
//  const { handleOutVarRenameChange, isVarUsedInNodes, removeUsedVarInNodes } = useWorkflow()
//
//  const { inputs, setInputs } = useNodeCrud<ContextAssignerNodeType>(id, payload)
...
const useConfig = (id: string, payload: ContextAssignerNodeType) => {
  ...
  const handleConversationVarChange = useCallback((value: string) => {
    setInputs({
      ...inputs,
      conversation_var: value,
    })
  }, [inputs, setInputs])

  const handleOperationChange = useCallback((value: ContextAssignerOperation) => {
    setInputs({
      ...inputs,
      operation: value,
      // Clear role and text if operation is set to clear
      ...(value === ContextAssignerOperation.Clear && {
        role: undefined,
        text: undefined,
      }),
    })
