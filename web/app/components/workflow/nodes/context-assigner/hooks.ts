...
import type {
  VarGroupItem,
  ContextAssignerNodeType,
} from './types'

export const useContextAssigner = () => {
  ...
  const handleAssignVariableValueChange = useCallback((nodeId: string, value: ValueSelector, varDetail: Var, groupId?: string) => {
    ...
    const node: Node<ContextAssignerNodeType> = nodes.find(node => node.id === nodeId)!
  ...
  const handleAddVariableInAddVariablePopupWithPosition = useCallback((
    ...
    contextAssignerNodeHandleId: string,
    value: ValueSelector,
    varDetail: Var,
  ) => {
    const {
    ...
    const newNodes = produce(getNodes(), (draft) => {
      draft.forEach((node) => {
        if (node.id === nodeId || node.id === contextAssignerNodeId) {
          node.data = {
            ...node.data,
    ...
    handleAssignVariableValueChange(contextAssignerNodeId, value, varDetail, contextAssignerNodeHandleId)
