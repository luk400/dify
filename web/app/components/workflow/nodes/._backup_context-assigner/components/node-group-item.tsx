...
const NodeGroupItem = ({
  ...
  const outputType = useMemo(() => {
    if (!groupEnabled)
      return item.contextAssignerNodeData.output_type

    const group = item.contextAssignerNodeData.advanced_settings?.groups.find(group => group.groupId === item.targetHandleId)
    return group?.output_type || ''
  }, [item.contextAssignerNodeData, item.targetHandleId, groupEnabled])
  const availableVars = getAvailableVars(item.contextAssignerNodeId, item.targetHandleId, filterVar(outputType as VarType), true)
  const showSelectionBorder = useMemo(() => {
    if (groupEnabled && enteringNodePayload?.nodeId === item.contextAssignerNodeId) {
    ...
    return false
  }, [enteringNodePayload, groupEnabled, hoveringAssignVariableGroupId, item.targetHandleId, item.contextAssignerNodeId])
      ...
      <div className='flex items-center justify-between h-4 text-[10px] font-medium text-gray-500'>
        ...
        <div className='flex items-center'>
          ...
          <AddVariable
            ...
            contextAssignerNodeId={item.contextAssignerNodeId}
            contextAssignerNodeData={item.contextAssignerNodeData}
