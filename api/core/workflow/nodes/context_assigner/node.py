...
class ContextAssignerNode(LLMNode):
    ...
    def _run(self) -> NodeRunResult:
        ...
        try:
            ...
            for var in self.node_data.conversation_variables:
                conversation_var = self.graph_runtime_state.variable_pool.get(var)
                if not isinstance(conversation_var, Variable):
                    raise VariableNotFoundError(variable_selector=[var[1]])
                ...
                match self.node_data.operation:
                    case Operation.ADD:
                        ...
                        if new_entry["role"] == "system":
                            ...
                            # there should be only one system message in the conversation
                            # first remove all system messages from the conversation, then prepend the new system message
                            current_conversation = [entry for entry in current_conversation if entry["role"] != "system"]
                            prompt_template.append(LLMNodeChatModelMessage(text=new_entry["text"], role=PromptMessageRole.SYSTEM, edition_type="basic"))
                        ...
                        if new_entry["role"] == "system":
                            updated_value = [new_entry] + current_conversation # system message should always be the first message
                        elif append:
                            updated_value = current_conversation + [new_entry]
                        else:
                            first_msg_is_system_prompt = (len(current_conversation) > 0) and (current_conversation[0]["role"] == "system")
                            if first_msg_is_system_prompt:
                                updated_value = [current_conversation[0]] + [new_entry] + current_conversation[1:]
                            else:
                                updated_value = [new_entry] + current_conversation

                    case Operation.CLEAR:
                        first_msg_is_system_prompt = (len(current_conversation) > 0) and (current_conversation[0]["role"] == "system")
                        updated_value = [current_conversation[0]] if first_msg_is_system_prompt else []

                    case Operation.REMOVE:
                        if not self.node_data.n or not self.node_data.which:
                            raise ValueError("N and which are required for remove operation")

                        current_conversation_without_system = [entry for entry in current_conversation if entry["role"] != "system"]
                        first_msg_is_system_prompt = (len(current_conversation) > 0) and (current_conversation[0]["role"] == "system")
                        
                        n = min(self.node_data.n, len(current_conversation_without_system))  # Don't remove more than exists

                        if n == 0:
                            updated_value = current_conversation
                        elif self.node_data.which == WhichEnd.FIRST:
                            if first_msg_is_system_prompt:
                                updated_value = [current_conversation[0]] + current_conversation[n:]
                            else:
